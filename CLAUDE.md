...
The deprecated TypeScript implementation that formerly lived under `src/` has been **removed** — the Rust port is the only implementation. The root `package.json` / `tsconfig.json` / `eslint.config.ts` remain only as repo JS tooling (lint/typecheck of `npm/`) and the release-please version anchor, and as scaffolding for the planned **napi-rs native-binding SDK** (which will bind the `crates/` Rust directly, not reintroduce a TS port).

**SDK packaging decision: keep the two distribution channels separate.** `npm/` stays the Biome-style CLI/MCP launcher — a thin Node shim that execs the **standalone Rust binary** (this preserves the no-runtime Homebrew story; do NOT convert it to napi). The napi-rs SDK is a distinct concern: an in-process native addon (a new crate, e.g. `crates/csp-node`, with `#[napi]` bindings over `crates/csp`) shipped as its **own npm package** (e.g. `@pleaseai/csp-sdk`), not merged into `npm/`. Both build outputs share the one `crates/csp` core.
