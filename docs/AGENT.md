...
- **Cloudflare** (if applicable) — `wrangler.jsonc` has `name`, `compatibility_date`, and `pages_build_output_dir = "./dist"` (a Cloudflare Pages project; `dist/404.html` is served for unknown paths, so no `assets` block).
